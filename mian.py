...
def Create_Grade_select(window):
    tk.Button(window,bg="white",text = "高中",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(9),Destroy_all_child(window),Grade_select_Second(window)]).pack()
    tk.Button(window,bg="white",text = "國中",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(6),Destroy_all_child(window),Grade_select_Second(window)]).pack()
    tk.Button(window,bg="white",text = "國小",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(0),Destroy_all_child(window),Grade_select_Second(window)]).pack()
    return None

def Grade_select_Second(window):
    if Grade < 6 :
        tk.Button(window,bg="white",text = "一年級",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(1),Destroy_all_child(window),Class_selector(window)]).pack()
        tk.Button(window,bg="white",text = "二年級",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(2),Destroy_all_child(window),Class_selector(window)]).pack()
        tk.Button(window,bg="white",text = "三年級",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(3),Destroy_all_child(window),Class_selector(window)]).pack()
        tk.Button(window,bg="white",text = "四年級",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(4),Destroy_all_child(window),Class_selector(window)]).pack()
        tk.Button(window,bg="white",text = "五年級",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(5),Destroy_all_child(window),Class_selector(window)]).pack()
        tk.Button(window,bg="white",text = "六年級",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Grade_select(6),Destroy_all_child(window),Class_selector(window)]).pack()
...
def Class_selector(window):
    if Grade <= 6:
        for i in const_class[0:4]:
            tk.Button(window,bg="white",text = i ,font =("Microsoft JhengHei",20,"bold"),command = lambda i=i:[Class_select(const_class.index(i)+1),Destroy_all_child(window),get_name(window)]).pack()
...
def get_name(window):
    ...
    input_name.pack()
    OK_btn = tk.Button(window,bg="white",text = "OK",font =("Microsoft JhengHei",20,"bold"),command = lambda:[Name_parser(input_name.get("1.0", "end")),Destroy_all_child(window),compitition_select(window)])
    OK_btn.pack()
...
def Creator(window,const,num,names,Grade):
    if len(const)>num:
        tk.Label(window,text = const[num] ,font =("Microsoft JhengHei",20,"bold")).grid(row=0,column=3)
        ...
        tk.Button(window,bg="white",text = "OK",font =("Microsoft JhengHei",20,"bold"),command = lambda:[push(var),Destroy_all_child(window),Creator(window,const,num+1,Names,Grade)]).grid(row=(len(Names)//7+2),column=3)
    elif len(const)==num:
        if Grade == 7:
...
def Creator_team(window,const,num,names):
    if len(const)>num:
        ...
        var = [tk.IntVar() for i in range(len(Names))]
        [tk.Checkbutton(window,bg="gray",text = name ,variable = var[Names.index(name)] ,font =("Microsoft JhengHei",20,"bold")).grid(row=(Names.index(name)//7+1),column=(Names.index(name)%7)) for name in Names]
        tk.Button(window,bg="white",text = "OK",font =("Microsoft JhengHei",20,"bold"),command = lambda:[push_team(var),Destroy_all_child(window),Creator_team(window,const,num+1,Names)]).grid(row=(len(Names)//7+2),column=3)
    elif len(const)==num:
        pass
